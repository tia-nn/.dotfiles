...
# env['COLOR_INPUT'] = True

# 結果のシンタックスハイライトをするかどうかのフラグ。
# env['COLOR_RESULTS'] = True
...
# env['DYNAMIC_CWD_WIDTH'] = (float('inf'), 'c')
...
# env['PROMPT_REFRESH_INTERVAL'] = 0.0

# プロンプトテンプレートのすべてのトークンを受け取る最終プロセッサ。他のトークンの値に基づいて異なるプレフィックスでフォーマットするオプションを提供します。
# powerline テーマのようなものを実装するのに便利。
# env['PROMPT_TOKENS_FORMATTER'] = xonsh.prompt.base.prompt_tokens_formatter_default

# 右側に整列したプロンプトテキスト用の template string。
# `$PROMPT` と同じように設定できる。現状は prompt-toolkit shell でのみ利用可能。
env['RIGHT_PROMPT'] = '{curr_branch} {env_name}'
...
env['SUGGEST_COMMANDS'] = False

# env['SUGGEST_MAX_NUM'] = 5
# env['SUGGEST_THRESHOLD'] = 3

env['SUPPRESS_BRANCH_TIMEOUT_MESSAGE'] = True

# env['TITLE'] = xonsh.environ.DEFAULT_TITLE

# env['UPDATE_PROMPT_ON_KEYPRESS'] = False

# env['VC_BRANCH_TIMEOUT'] = 0.1
# env['VC_GIT_INCLUDE_UNTRACKED'] = False
env['VC_HG_SHOW_BRANCH'] = False

env['XONSH_HISTORY_MATCH_ANYWHERE'] = True

# env['XONSH_STDERR_PREFIX'] = ''
# env['XONSH_STDERR_POSTFIX'] = ''
